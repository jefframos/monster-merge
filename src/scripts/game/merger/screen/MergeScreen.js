...
import UnicornBackground from '../backgrounds/unicorn/UnicornBackground';
...
export default class MergeScreen extends Screen {
    constructor(label) {
        ...
        window.baseConfigGameUnicorns = PIXI.loader.resources['baseConfigUnicorn'].data.baseGame;
        ...
        window.baseUnicorns = PIXI.loader.resources['unicorns'].data;
        ...
        this.registerSystem(containers, window.baseConfigGameUnicorns, window.baseUnicorns, 'unicorns', true, new UnicornBackground(), true)
    ...
    registerSystem(containers, baseData, baseMergeData, slug, visible = true, interactiveBackground = null, forceActive = false) {
        ...
        let toggleSystems = new UIButton1(0x002299, rawMergeDataList[0].rawData.imageSrc, 0xFFFFFF, buttonSize, buttonSize, forceActive ? config.assets.button.tertiarySquare : config.assets.button.primarySquare)
    ...
    onMergeSystemUpdate(data) {
        ...
        for (let index = 1; index < this.systemsList.length; index++) {
            ...
            if (!element.forceActive && (!prev.boardProgression || prev.boardProgression.currentLevel < 6) && !window.allUnlock) {
                element.toggle.disable()
            } else {
                if (!isInit && !element.forceActive) {
